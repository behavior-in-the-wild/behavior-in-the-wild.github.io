/* leaderboard.html — full sortable table */
(function () {
  "use strict";
  var S = window.SITW;
  ...
  var COLS = [
    ...
    { key: "accuracy", label: "Accuracy", num: true, sortable: true, bar: true },
    { key: "skill_pp", label: "Skill vs consensus", num: true, sortable: true },
    { key: "brier", label: "Calibration (Brier↓)", num: true, sortable: true },
  ...
  function render() {
    ...
    COLS.forEach(function (col) {
      ...
      if (col.sortable) {
        var ar = S.el("span", "arrow", col.key === sortKey ? (sortDir === 1 ? "▲" : "▼") : "↕");
        th.appendChild(ar);
        th.addEventListener("click", function () {
          if (sortKey === col.key) { sortDir = -sortDir; }
          else { sortKey = col.key; sortDir = (col.key === "model" || col.key === "type") ? 1 : (col.key === "brier" || col.key === "rank" ? 1 : -1); }
          render();
        });
      }
      htr.appendChild(th);
    ...
    sorted.forEach(function (r) {
      ...
      var nm = S.el("div");
      var nameLine = S.el("div", "mc-name");
      nameLine.appendChild(document.createTextNode(r.model + " "));
      if (r.pending) nameLine.appendChild(S.el("span", "tag tag-pending", "Pending"));
